...
class Class(db.Model):
    ...
    description = db.Column(db.Text, nullable=True)
    professor_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    enrollment_code = db.Column(db.String(50), unique=True, nullable=False) # Code for students to join

    # Relationship to the User model (one professor creates many classes)
...
# --- NOVAS ROTAS DE CLASSE E MATRÍCULA ---
...
def create_class():
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} tentando criar uma nova turma.")
    
    user = User.query.get(current_user_id)

    if not user or user.role != 'professor':
        app.logger.warning(f"Acesso negado para ID {current_user_id} na criação de turma. Role: {user.role if user else 'N/A'}")
        return jsonify({"message": "Acesso negado. Apenas professores podem criar turmas."}), 403

    data = request.get_json()
    app.logger.debug(f"Dados recebidos para nova turma: {data}")

    name = data.get('name')
    description = data.get('description')

    if not name:
        app.logger.warning(f"Tentativa de criar turma pelo ID {current_user_id} falhou: nome ausente.")
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(f"Turma '{new_class.name}' (ID: {new_class.id}, Code: {new_class.enrollment_code}) criada com sucesso pelo usuário ID {current_user_id}.")

        return jsonify({"message": "Turma criada com sucesso!", "class": new_class.to_dict()}), 201

    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao criar turma para o usuário ID {current_user_id}: {str(e)}", exc_info=True)
...
def get_classes():
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} solicitou a lista de turmas.")
    
    user = User.query.get(current_user_id)

    if not user:
        app.logger.error(f"Tentativa de listar turmas para usuário inexistente: ID {current_user_id}")
        return jsonify({"message": "Usuário não encontrado."}), 404

    if user.role == 'professor':
        classes = Class.query.filter_by(professor_id=current_user_id).all()
        classes_data = [cls.to_dict() for cls in classes]
        app.logger.info(f"Retornando {len(classes_data)} turmas criadas pelo professor ID {current_user_id}.")
        ...
        app.logger.info(f"Retornando {len(enrolled_classes_data)} turmas em que o aluno ID {current_user_id} está matriculado.")
        return jsonify(enrolled_classes_data), 200
    else:
        app.logger.warning(f"Acesso negado à lista de turmas para o ID {current_user_id}. Role: {user.role}")
...
def get_class_details(class_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} solicitou detalhes da turma ID {class_id}.")

    user = User.query.get(current_user_id)
    class_obj = Class.query.get(class_id)

    if not user:
        app.logger.error(f"Tentativa de ver detalhes da turma ID {class_id} para usuário inexistente: ID {current_user_id}")
        return jsonify({"message": "Usuário não encontrado."}), 404
    
    if not class_obj:
        app.logger.warning(f"Turma ID {class_id} não encontrada para o usuário ID {current_user_id}.")
    ...
    if not (is_professor or is_student):
        app.logger.warning(f"Acesso negado aos detalhes da turma ID {class_id} para o usuário ID {current_user_id}. Não é professor nem aluno matriculado.")
    ...
    app.logger.debug(f"Detalhes da turma ID {class_id} recuperados para o usuário ID {current_user_id}.")
...
def update_class(class_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Professor ID {current_user_id} tentando atualizar a turma ID {class_id}.")

    user = User.query.get(current_user_id)
    class_to_update = Class.query.get(class_id)

    if not user or user.role != 'professor':
        app.logger.warning(f"Acesso negado para ID {current_user_id} ao tentar atualizar turma ID {class_id}. Role: {user.role if user else 'N/A'}")
        return jsonify({"message": "Acesso negado. Apenas professores podem atualizar turmas."}), 403

    if not class_to_update:
        app.logger.error(f"Professor ID {current_user_id} tentou atualizar uma turma inexistente: ID {class_id}")
        return jsonify({"message": "Turma não encontrada."}), 404

    if class_to_update.professor_id != user.id:
        app.logger.warning(f"Acesso negado para professor ID {current_user_id} ao tentar atualizar turma ID {class_id}. Não é o criador.")
        return jsonify({"message": "Acesso negado. Você não é o professor responsável por esta turma."}), 403

    data = request.get_json()
    app.logger.debug(f"Dados recebidos para atualização da turma ID {class_id}: {data}")
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(f"Turma ID {class_id} atualizada com sucesso pelo professor ID {current_user_id}.")
        return jsonify({"message": "Turma atualizada com sucesso!", "class": class_to_update.to_dict()}), 200
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao atualizar turma ID {class_id}: {str(e)}", exc_info=True)
...
def delete_class(class_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Professor ID {current_user_id} tentando deletar a turma ID {class_id}.")

    user = User.query.get(current_user_id)
    class_to_delete = Class.query.get(class_id)

    if not user or user.role != 'professor':
        app.logger.warning(f"Acesso negado para ID {current_user_id} ao tentar deletar turma ID {class_id}. Role: {user.role if user else 'N/A'}")
        return jsonify({"message": "Acesso negado. Apenas professores podem deletar turmas."}), 403

    if not class_to_delete:
        app.logger.error(f"Professor ID {current_user_id} tentou deletar uma turma inexistente: ID {class_id}")
        return jsonify({"message": "Turma não encontrada."}), 404

    if class_to_delete.professor_id != user.id:
        app.logger.warning(f"Acesso negado para professor ID {current_user_id} ao tentar deletar turma ID {class_id}. Não é o criador.")
        return jsonify({"message": "Acesso negado. Você não é o professor responsável por esta turma."}), 403
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(f"Turma ID {class_id} e suas inscrições/atividades associadas (se houver) deletadas/desassociadas com sucesso pelo professor ID {current_user_id}.")
        return jsonify({"message": "Turma deletada com sucesso!"}), 200
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao deletar turma ID {class_id}: {str(e)}", exc_info=True)
...
def join_class():
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} tentando entrar em uma turma.")

    user = User.query.get(current_user_id)

    if not user or user.role != 'aluno':
        app.logger.warning(f"Acesso negado para ID {current_user_id} na entrada de turma. Role: {user.role if user else 'N/A'}")
        return jsonify({"message": "Acesso negado. Apenas alunos podem entrar em turmas."}), 403

    data = request.get_json()
    enrollment_code = data.get('enrollment_code')
    app.logger.debug(f"Aluno ID {current_user_id} tentou entrar com código: {enrollment_code}")

    if not enrollment_code:
        app.logger.warning(f"Tentativa de entrada em turma pelo ID {current_user_id} falhou: código ausente.")
    ...
    if not class_to_join:
        app.logger.warning(f"Tentativa de entrada em turma pelo ID {current_user_id} falhou: código '{enrollment_code}' inválido.")
    ...
    if existing_enrollment:
        app.logger.warning(f"Aluno ID {current_user_id} já está matriculado na turma ID {class_to_join.id}.")
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(f"Aluno ID {current_user_id} matriculado com sucesso na turma '{class_to_join.name}' (ID: {class_to_join.id}).")
        return jsonify({"message": "Matrícula realizada com sucesso!", "class": class_to_join.to_dict()}), 201
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao matricular aluno ID {current_user_id} na turma ID {class_to_join.id}: {str(e)}", exc_info=True)
...
def leave_class(class_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} tentando sair da turma ID {class_id}.")

    user = User.query.get(current_user_id)

    if not user or user.role != 'aluno':
        app.logger.warning(f"Acesso negado para ID {current_user_id} ao sair de turma. Role: {user.role if user else 'N/A'}")
    ...
    if not enrollment_to_delete:
        app.logger.warning(f"Aluno ID {current_user_id} não está matriculado na turma ID {class_id}.")
        return jsonify({"message": "Você não está matriculado nesta turma."}), 404

    try:
        db.session.delete(enrollment_to_delete)
        db.session.commit()
        app.logger.info(f"Aluno ID {current_user_id} saiu com sucesso da turma ID {class_id}.")
        return jsonify({"message": "Você saiu da turma com sucesso."}), 200
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao sair da turma ID {class_id} para o aluno ID {current_user_id}: {str(e)}", exc_info=True)
        return jsonify({"message": f"Erro interno do servidor ao sair da turma: {str(e)}"}), 500

@app.route('/api/activities/<int:activity_id>/assign', methods=['POST'])
@cross_origin()
@jwt_required()
def assign_activity_to_class(activity_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} tentando atribuir atividade ID {activity_id} a uma turma.")

    user = User.query.get(current_user_id)
    activity = Activity.query.get(activity_id)

    if not user or user.role != 'professor':
        app.logger.warning(f"Acesso negado para ID {current_user_id} na atribuição de atividade. Role: {user.role if user else 'N/A'}")
        return jsonify({"message": "Acesso negado. Apenas professores podem atribuir atividades."}), 403

    if not activity:
        app.logger.error(f"Atividade ID {activity_id} não encontrada para atribuição pelo professor ID {current_user_id}.")
        return jsonify({"message": "Atividade não encontrada."}), 404

    if activity.professor_id != user.id:
        app.logger.warning(f"Acesso negado para professor ID {current_user_id} ao tentar atribuir atividade ID {activity_id}. Não é o criador.")
        return jsonify({"message": "Acesso negado. Você não é o professor responsável por esta atividade."}), 403

    data = request.get_json()
    class_id = data.get('class_id')
    app.logger.debug(f"Atribuindo atividade ID {activity_id} à turma ID {class_id}.")

    if not class_id:
        app.logger.warning(f"Tentativa de atribuição de atividade pelo ID {current_user_id} falhou: class_id ausente.")
        return jsonify({"message": "O ID da turma é obrigatório para atribuir a atividade."}), 400

    target_class = Class.query.get(class_id)
    if not target_class:
        app.logger.warning(f"Tentativa de atribuição de atividade pelo ID {current_user_id} falhou: turma ID {class_id} não encontrada.")
        return jsonify({"message": "Turma não encontrada."}), 404

    if target_class.professor_id != user.id:
        app.logger.warning(f"Acesso negado para professor ID {current_user_id} ao tentar atribuir atividade a turma ID {class_id}. Não é o professor da turma.")
        return jsonify({"message": "Acesso negado. Você não é o professor responsável por esta turma."}), 403

    try:
        activity.class_id = class_id
        db.session.commit()
        app.logger.info(f"Atividade '{activity.title}' (ID: {activity_id}) atribuída com sucesso à turma '{target_class.name}' (ID: {class_id}).")
        return jsonify({"message": "Atividade atribuída à turma com sucesso!", "activity": activity.to_dict()}), 200
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Erro ao atribuir atividade ID {activity_id} à turma ID {class_id}: {str(e)}", exc_info=True)
        return jsonify({"message": f"Erro interno do servidor ao atribuir atividade à turma: {str(e)}"}), 500

@app.route('/api/classes/<int:class_id>/activities', methods=['GET'])
@cross_origin()
@jwt_required()
def get_class_activities(class_id):
    current_user_id = get_jwt_identity()
    app.logger.info(f"Usuário ID {current_user_id} solicitou atividades para a turma ID {class_id}.")

    user = User.query.get(current_user_id)
    class_obj = Class.query.get(class_id)

    if not user:
        app.logger.error(f"Tentativa de listar atividades de turma para usuário inexistente: ID {current_user_id}")
        return jsonify({"message": "Usuário não encontrado."}), 404

    if not class_obj:
        app.logger.warning(f"Turma ID {class_id} não encontrada ao listar atividades para o usuário ID {current_user_id}.")
        return jsonify({"message": "Turma não encontrada."}), 404
    ...
    if not (is_professor or is_student):
        app.logger.warning(f"Acesso negado às atividades da turma ID {class_id} para o usuário ID {current_user_id}. Não é professor nem aluno matriculado.")
    ...
    activities_data = [activity.to_dict() for activity in activities]
    app.logger.info(f"Retornando {len(activities_data)} atividades para a turma ID {class_id} para o usuário ID {current_user_id}.")
    return jsonify(activities_data), 200
