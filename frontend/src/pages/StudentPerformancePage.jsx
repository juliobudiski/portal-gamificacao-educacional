...
const StatusBadge = ({ status }) => {
  const styles = {
    ...
    not_started: "bg-primary-bg text-secondary-text border border-border-color",
  };
...
const ScoreDistributionChart = ({ students, maxScore }) => {
  ...
  return (
    <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg mb-6">
      <h3 className="text-sm font-semibold text-primary-text mb-3 flex items-center gap-2">
        ...
        <span className="text-xs font-normal text-secondary-text">(Baseado no total de {safeMaxScore} pontos)</span>
      </h3>
      <div className="flex h-4 w-full rounded-full overflow-hidden bg-primary-bg border border-border-color">
...
const StudentPerformancePage = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 min-h-screen bg-primary-bg text-primary-text transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-accent-purple/20 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      {/* Filtros */}
      <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg mb-8 flex flex-col md:flex-row gap-6 items-end md:items-center justify-between transition-colors duration-300">
        <div className="flex flex-col md:flex-row gap-6 w-full md:w-auto">
          <div className="flex flex-col">
            <label className="text-sm font-medium text-secondary-text mb-2">Turma</label>
            <select
              className="px-4 py-2 bg-primary-bg border border-border-color text-primary-text rounded-lg focus:ring-2 focus:ring-accent-teal focus:outline-none transition-all shadow-inner"
              value={selectedClass}
              onChange={(e) => setSelectedClass(e.target.value)}
            >
              <option value="" disabled className="bg-secondary-bg text-primary-text">Selecione uma turma</option>
              {classes.map(c => <option key={c.id} value={c.id} className="bg-secondary-bg text-primary-text">{c.name}</option>)}
            </select>
          </div>
          <div className="flex flex-col">
            <label className="text-sm font-medium text-secondary-text mb-2">Atividade</label>
            <select
              className="px-4 py-2 bg-primary-bg border border-border-color text-primary-text rounded-lg focus:ring-2 focus:ring-accent-teal focus:outline-none transition-all shadow-inner"
              value={selectedActivity}
              onChange={(e) => setSelectedActivity(e.target.value)}
            >
              <option value="" className="bg-secondary-bg text-primary-text">Visão Geral (Todas)</option>
              {activities.map(a => <option key={a.id} value={a.id} className="bg-secondary-bg text-primary-text">{a.title}</option>)}
            </select>
          </div>
        </div>
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-primary-bg border border-border-color rounded-lg pl-10 pr-4 py-2 text-primary-text placeholder:text-secondary-text focus:ring-2 focus:ring-accent-teal outline-none transition-all shadow-inner"
            value={searchTerm}
      ...
      {!loading && performanceData?.stats?.total_students > 0 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg flex items-center gap-4 hover:border-accent-teal/50 transition-colors">
              <div className="p-3 rounded-full bg-blue-500/20 text-blue-500 dark:text-blue-400">
            ...
            {/* 2. Pontuação Média */}
            <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg flex items-center gap-4 hover:border-accent-purple/50 transition-colors">
            ...
            <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg flex items-center gap-4 hover:border-success/50 transition-colors">
            ...
            <div className="bg-secondary-bg border border-border-color p-6 rounded-xl shadow-lg flex items-center gap-4 hover:border-accent-yellow/50 transition-colors">
              <div className="p-3 rounded-full bg-accent-yellow/20 text-accent-yellow">
                <CheckCircle size={28} />
              </div>
              <div>
                <p className="text-xs text-secondary-text uppercase font-bold tracking-wider">Alunos Concluídos</p>
                <p className="text-3xl font-bold text-accent-yellow mt-1">
                  {performanceData.stats.completed_count} <span className="text-lg text-secondary-text font-normal">/ {performanceData.stats.total_students}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      {/* TABELA DE ALUNOS */}
      <div className="bg-secondary-bg border border-border-color rounded-xl shadow-lg overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-secondary-text">Carregando dados...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-left">
              <thead className="bg-primary-bg text-secondary-text uppercase text-xs font-bold border-b border-border-color tracking-wider">
                <tr>
                  <th className="p-4 px-6">Aluno</th>
                  {selectedActivity ? (
                    <>
                      <th className="p-4 text-center">Status</th>
                      <th className="p-4 text-center text-success font-bold" title="Questões respondidas corretamente">Acertos</th>
                      <th className="p-4 text-center text-danger font-bold" title="Questões respondidas incorretamente">Erros</th>
              ...
              <tbody className="divide-y divide-border-color">
                {performanceData?.students && performanceData.students.length > 0 ? (
                  performanceData.students.map((student) => (
                    <tr key={student.id} className="hover:bg-hover-bg-color transition-colors group">
                      <td className="p-4 px-6 flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-primary-bg border border-border-color overflow-hidden text-secondary-text flex-shrink-0">
                        ...
                        <div>
                          <p className="font-semibold text-primary-text group-hover:text-accent-teal transition-colors">{student.name}</p>
                      ...
                      {selectedActivity ? (
                        <>
                          ...
                          {/* ERROS */}
                          <td className="p-4 text-center">
                            <div className="flex items-center justify-center gap-1 text-danger font-bold">
                          ...
                          {/* PRECISÃO */}
                          <td className="p-4 text-center">
                            <span className={`font-mono font-bold ${student.accuracy >= 70 ? 'text-success' : student.accuracy >= 50 ? 'text-accent-yellow' : 'text-danger'}`}>{student.accuracy}%</span>
                          </td>
                        ...
                        <>
                          ...
                          <td className="p-4 px-6 text-right text-sm text-secondary-text">{student.last_location || 'Desconhecido'}</td>
                        </>
                      )}
                    </tr>
                  ))
