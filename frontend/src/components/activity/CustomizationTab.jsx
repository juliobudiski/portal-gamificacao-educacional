...
const CustomizationTab = ({ activityId, onReturn, onCustomizationChange }) => {
    ...
    return (
        <div className="relative pt-16 w-full max-w-6xl mx-auto p-8 mb-12 text-primary-text animate-fade-in bg-primary-bg rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-white/10 overflow-hidden">
            ...
            <header className="text-center mb-10 pt-4 relative z-10">
                ...
                <p className="text-secondary-text mt-2">Personalize sua identidade e exiba suas conquistas.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
                ...
                <div className="lg:col-span-1 bg-black/40 backdrop-blur-md p-6 rounded-2xl border border-white/10 self-start shadow-inner">
                    ...
                    <div className="bg-gradient-to-b from-gray-800 to-black p-6 rounded-xl flex flex-col items-center gap-6 border border-white/5 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative">
                            ...
                            <img src={equipped.avatarUrl || user.profile_picture || '/avatars/default_avatar.webp'} alt="Avatar" className="relative w-32 h-32 rounded-full object-cover border-4 border-[var(--border-color)] shadow-[0_0_20px_rgba(0,0,0,0.8)] z-10" />
                        </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-black/30 backdrop-blur-sm p-6 rounded-2xl border border-white/5 shadow-lg">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button onClick={() => handleEquip('/equip-title', { title_id: null }, () => setEquipped(p => ({ ...p, titleId: null })))}
                                className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 ${!equipped.titleId ? 'bg-gradient-to-r from-yellow-600 to-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.5)]' : 'bg-secondary-bg text-secondary-text border border-[var(--border-color)] hover:bg-hover-bg-color0 hover:text-white'}`}>
                                Sem Título
                            </button>
                            {unlockedTitles.map(title => (
                                <button key={title.id} onClick={() => handleEquip('/equip-title', { title_id: title.id }, () => setEquipped(p => ({ ...p, titleId: title.id })))}
                                    className={`px-5 py-2 rounded-xl text-sm font-bold transition-all duration-300 shadow-md ${equipped.titleId === title.id ? 'bg-gradient-to-r from-yellow-600 to-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.5)]' : 'bg-secondary-bg text-secondary-text border border-[var(--border-color)] hover:bg-hover-bg-color0 hover:text-white'}`}>
                                    {title.displayText}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="bg-black/30 backdrop-blur-sm p-6 rounded-2xl border border-white/5 shadow-lg">
                        ...
                        <p className="text-sm text-secondary-text mb-6 font-medium bg-gray-800/50 p-3 rounded-lg border border-gray-700/50">
                        ...
                        <div className="flex flex-wrap gap-4 border-b border-gray-800 pb-6 mb-6">
                            {unlockedCosmetics.map(cosmetic => (
                                <button key={cosmetic.id} onClick={() => setSelectedCosmetic(cosmetic)}
                                    className={`px-4 py-3 rounded-xl border-2 transition-all duration-300 font-bold tracking-wide shadow-md ${selectedCosmetic?.id === cosmetic.id ? 'border-pink-500 bg-pink-500/10 scale-105 shadow-[0_0_15px_rgba(236,72,153,0.4)]' : 'border-[var(--border-color)] bg-gray-900/50 hover:border-gray-500'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-gray-900/80 p-5 rounded-2xl border border-gray-700/50 shadow-inner flex flex-col justify-between">
                                <div>
                                    <h4 className="font-bold text-secondary-text mb-2 uppercase text-xs tracking-wider">Aplicar ao Nome</h4>
                                ...
                                <button onClick={() => handleEquipCosmetic('name')} disabled={!selectedCosmetic || isSaving}
                                    className={`w-full py-3 mt-4 rounded-xl font-bold transition-all duration-300 shadow-lg ${selectedCosmetic?.id === equipped.nameCosmeticId 
                                        ...
                                        : 'bg-emerald-600 hover:bg-emerald-500 text-white disabled:bg-secondary-bg disabled:text-gray-600 disabled:border-none border border-emerald-400/50'}`}>
                            ...
                            <div className="bg-gray-900/80 p-5 rounded-2xl border border-gray-700/50 shadow-inner flex flex-col justify-between">
                                <div>
                                    <h4 className="font-bold text-secondary-text mb-2 uppercase text-xs tracking-wider">Aplicar ao Título</h4>
                                    <div className="my-4 h-20 flex items-center justify-center bg-black/60 rounded-xl border border-black shadow-inner overflow-hidden">
