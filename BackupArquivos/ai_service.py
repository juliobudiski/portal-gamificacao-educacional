...
class AIService:
    def __init__(self):
        ...
        # --- LISTA DE PRIORIDADE (Do melhor para o "quebra-galho") ---
        # Baseado na lista que você rodou no check_models.py
    ...
    def orchestrate_story(self, activity_context: Dict, path_structure: List[Dict], ai_config: Dict, client_socket_id: str = None) -> Dict:
        ...
        if len(steps_to_fill) > 15:
            # Ou corta a lista, ou retorna erro pedindo para dividir
        ...
        for index, step in enumerate(steps_to_fill):
            ...
            while attempts < MAX_RETRIES and not valid_content:
                ...
                try:
                    ...
                    if "429" in error_msg:
                        wait_time = (attempts + 1) * 10 # Espera 20s, 30s...
                        if client_socket_id:
                            socketio.emit('ai_progress', {
                                'percent': int(((index) / total_steps) * 100),
            ...
            # --- FREIO DE MÃO (Cool-down) ---
            # Evita disparar o próximo passo imediatamente se não for o último
            if index < total_steps - 1:
                cooldown = 10 # 10 segundos de resfriamento garantido
                logger.info(f"⏳ Resfriando API por {cooldown}s para evitar Rate Limit...")
                time.sleep(cooldown)
    ...
    def _generate_content_with_fallback(self, prompt: str) -> str:
        ...
        for model_name in self.MODEL_HIERARCHY:
            try:
                ...
                error_str = str(e)
                # Se for erro de cota (429), loga e passa para o próximo loop
    ...
    def _shuffle_quiz_options(self, content: Dict) -> Dict:
        ...
        for q in content['questions']:
            ...
            # 2. Limpa todas as opções
            clean_opts = [clean_text(o) for o in raw_options]
            
            # 3. Limpa a resposta correta para garantir o match
            clean_correct = clean_text(raw_correct)
            
            # Se a resposta correta não estiver na lista limpa (caso raro de alucinação),
            # tentamos achar pelo índice se ela era "Opção A"
            if clean_correct not in clean_opts:
                # Tenta match parcial ou usa a primeira como fallback de segurança
                # (Isso evita crash se a IA alucinar uma resposta que não está nas opções)
                clean_correct = clean_opts[0] 

            # 4. Cria pares (Opção, É_Correta?)
            ...
            # Encontra quem é a correta agora
